...
# The full bootstrap DCS string the server emits:  ESC P +Hraw ESC \
BOOTSTRAP_RE = /#{Regexp.escape(Protocol::HTTY::Stream::DCS + Protocol::HTTY::Stream::BOOTSTRAP_PREFIX + Protocol::HTTY::Stream::RAW_MODE + Protocol::HTTY::Stream::ST)}/n
...
#
# This wrapper exists because Protocol::HTTY::Stream is designed for use with
# already-raw IO (server side or pipe pairs) and reads one byte at a time for
# DCS scanning. The client side needs chunk-based PTY buffering plus the
# multi-session reset! dance, which are handled here.
class PTYStream
	def initialize(r, w)
		@r = r
		@w = w

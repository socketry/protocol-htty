...
Closing a local HTTY stream abstraction does not by itself imply closing the underlying terminal input or output stream. Implementations MAY stop writing HTTY chunks through a local stream object while leaving the surrounding terminal transport open for unrelated terminal traffic. Only actual terminal or PTY EOF on a transport direction terminates that HTTY transport direction.
...
Implementations that expose stream-like wrappers MAY model `close` as local write shutdown only, provided this does not implicitly close terminal stdin, stdout, or any broader session transport.
